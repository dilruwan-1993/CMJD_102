...
import com.cruds.model.Customer;
...
public class DialogAddStudent extends javax.swing.JDialog {
    ...
    private void btnAddStudentActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddStudentActionPerformed
        usn = Integer.parseInt(txtUSN.getText().trim());
        String name = txtNAME.getText().trim();
        
        if(name.length() == 0 || usn == 0)
        {
            JOptionPane.showMessageDialog(rootPane, "Please enter Name and ID", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        ProductDAO dao = new ProductDAO();

        if (dao.customerExist(new Customer(usn, name))) {
            JOptionPane.showMessageDialog(rootPane, "Student already Exist", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        } else {
            dao.addCustomer(new Customer(usn, name));
